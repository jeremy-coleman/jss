...
export default function jssIsolate(options = {}) {
    ...
    function onProcessRule(rule, sheet) {
        if (!sheet || sheet === resetSheet || rule.type !== 'style')
            return;
        // Type it as a StyleRule
        const styleRule = rule;
        if (!shouldIsolate(styleRule, sheet, options))
            return;
    ...
    function onProcessSheet() {
        if (!setSelectorDone && selectors.length)
            setSelector();
    }
    return {
        onProcessRule,
