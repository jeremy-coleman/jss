...
import { Plugin, StyleRule, StyleSheet } from 'jss';
...
export default function jssNested(): Plugin {
  ...
  function onProcessStyle(style, rule, sheet?: StyleSheet) {
    ...
    const container: StyleSheet = (styleRule.options.parent as any)
    let options
    let replaceRef
    for (const prop in style) {
      ...
      if (isNested) {
        ...
        container
          ...
          //actually i think the code is broken, upgrading from 9 to 10 breaks plugins using new StyleRule() call
