...
const createStyle = () => {
  const el = document.createElement('style'); // Without it, IE will have a broken source order specificity if we
...
export default class DomRenderer {
  getPropertyValue: (cssRule: any, prop: any) => any;
  setProperty: (cssRule: any, prop: any, value: any) => boolean;
  removeProperty: (cssRule: any, prop: any) => void;
  setSelector: (cssRule: any, selectorText: any) => boolean;
  element: any;
  sheet: any;
  hasInsertedRules: boolean;
  ...
  constructor(sheet) {
    ...
    //this is a terrible patern anyway, ..use this.sheet.options || Object.assign(this, {media: '', meta: '', element: ''})
  ...
  insertRule(rule, index, nativeParent = this.element.sheet) {
    if (rule.rules) {
      ...
      if (rule.type === 'conditional' || rule.type === 'keyframes') {
        ...
        latestNativeParent = insertRule(nativeParent, parent.toString({
          children: false
        }), index);
