...
function objectToArray(value, prop, rule, isFallback, isInArray) {
  ...
  const result = []; // Check if exists any non-standard property
...
function customPropsToStyle(value, rule, customProps, isFallback) {
  for (const prop in customProps) {
    const propName = customProps[prop]; // If current property doesn't exist already in rule - add new one

    if (typeof value[prop] !== 'undefined' && (isFallback || !rule.prop(propName))) {
      const appendedValue = styleDetector({
        [propName]: value[prop]
      }, rule)[propName]; // Add style directly in rule
